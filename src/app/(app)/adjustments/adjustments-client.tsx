...
import { canApproveAdjustments, canAutoApproveAdjustments } from "@/lib/permissions"
...
function StatusBadge({ status }: { status: string }) {
  if (status === "approved")
    return <Badge className="bg-green-100 text-green-700 hover:bg-green-100 text-xs font-medium px-2.5 py-0.5">Approved</Badge>
  if (status === "rejected")
    return <Badge className="bg-red-100 text-red-700 hover:bg-red-100 text-xs font-medium px-2.5 py-0.5">Rejected</Badge>
  return <Badge className="bg-amber-100 text-amber-700 hover:bg-amber-100 text-xs font-medium px-2.5 py-0.5">Pending</Badge>
}

export function AdjustmentsClient({ adjustments, branchProducts, session, userNames }: Props) {
  ...
  const autoApprove = session.role ? canAutoApproveAdjustments(session.role) : false
  ...
  async function handleSubmit() {
    ...
    if (autoApprove) {
      ...
      if (!bpErr && currentBp) {
        const currentStock = ut === "kg" ? currentBp.current_stock_kg : currentBp.current_stock_units
        const newStock =
          adjType === "increase"
            ? currentStock + qty
        ...
        await supabase
          .from("branch_products")
          .update({ [stockCol]: newStock, updated_at: new Date().toISOString() })
          .eq("id", bpId)
      }
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm table-fixed">
            ...
            <thead className="bg-muted/50 border-b">
              <tr>
                ...
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Delta (kg/units)</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Status</th>
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">By</th>
                ...
                adjustments.map((adj) => {
                  ...
                  return (
                    <tr key={adj.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-4 py-3.5 font-medium truncate">{adj.product?.name ?? "—"}</td>
                      <td className="px-4 py-3.5 text-muted-foreground">{formatAdjustmentReason(adj.reason)}</td>
                      <td className="px-4 py-3.5 text-muted-foreground text-xs truncate">{adj.notes || "—"}</td>
                      <td className={`px-4 py-3.5 font-mono font-semibold ${isIncrease ? "text-green-600" : "text-red-500"}`}>
                        {formatDelta(adj)}
                      </td>
                      <td className="px-4 py-3.5"><StatusBadge status={adj.status} /></td>
                      <td className="px-4 py-3.5 text-muted-foreground truncate">{resolveAdjusterName(adj)}</td>
                      <td className="px-4 py-3.5 text-muted-foreground text-xs whitespace-nowrap">{formatDate(adj.created_at)}</td>
                      <td className="px-4 py-3.5">
                        {canAct ? (
                          ...
                          <span className="text-muted-foreground/40 text-xs block text-center">—</span>
                        )}
                      </td>
                    </tr>
                  )
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setBpId(""); setQuantity(""); setNotes(""); setReason("recount_correction"); setAdjType("increase") } }}>
          ...
          <div className="space-y-4 mt-1">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={reason} onValueChange={(v) => setReason(v as AdjustmentReason)}>
                ...
                <SelectContent>
                  {REASONS.map((r) => (
                    <SelectItem key={r} value={r}>{formatAdjustmentReason(r)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            {!autoApprove && (
              <p className="text-xs text-amber-600 bg-amber-50 rounded-md px-3 py-2 border border-amber-100">
                This adjustment will be sent to the GM or Owner for approval before it affects stock.
              </p>
            )}
